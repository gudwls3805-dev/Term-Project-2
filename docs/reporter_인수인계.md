...
| 위 리포트에 **AI 추출 결과 포함** | ⚠️ **부분** — 자리와 코드는 완성, 실제 AI 결과로 검증한 적 없음. 사유는 6-1 |
...
| 위 리포트에 **AI 추출 결과 포함** | ⚠️ **부분** | B·D 연결 대기 |
...
리스크는 낮다 — D가 `main.py`에서 한 줄 연결하면 채워진다(8-1 참고).
다만 **연결해서 눈으로 보기 전까지는 "완료"라고 적지 않는다.**
